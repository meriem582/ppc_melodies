...
class ProviderActor (db: ActorRef) extends Actor{ 
    ...
    def receive : Receive = {
        case GetMeasureResult(i) => {
            lastSender = Some(sender())
            val res = resultatCalcule(i)
            db ! GetMeasure(res)
        }
        case measure : Measure => {
            lastSender.foreach(_ ! measure)
        }
    }

    def resultatCalcule(i: Int): Int = {
      val partie = if ((compteur / 16) % 2 == 0) partie1 else partie2

      val rowIndex = i - 2
      val colIndex = compteur % 8
      compteur += 1

      partie(rowIndex)(colIndex) - 1 
    }
